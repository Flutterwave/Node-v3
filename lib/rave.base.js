...
var RaveBase = function (public_key, secret_key, _base_url) {
  ...
  this.request = function (path, payload, callback) {
    ...
    if (callback) {
      telemetry.logRequest(requestMethod, path);
...
RaveBase.prototype._makeRequest = function (url, fetchOptions, callback) {
  fetch(url, fetchOptions)
      ...
      return response
        ...
        .then((body) => {
          if (body.status === 'error') {
            telemetry.logError(body.code || 'API_ERROR', body.message);
          }
    ...
    .catch((err) => {
      telemetry.logError(err.code || 'REQUEST_FAILED', err.message);
      callback(err, {}, {});
    });

  telemetry.logRequest(fetchOptions.method, path);
};
